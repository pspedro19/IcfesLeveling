...
class IntelligentVideoMatchingService:
    ...
    def get_video_embeddings(self, subject_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            video_query = text("""
                ...
                ORDER BY xp_reward DESC NULLS LAST, difficulty_level ASC NULLS LAST
            """)
            ...
            for video in videos:
                video_dict = {
                    'video_id': video[0],  # id
                    'title': video[1],  # title
                    'description': video[2] or '',  # description
                    'tema_principal': video[3] or '',  # topic
                    'duration_seconds': int(video[4] * 60) if video[4] else 0,  # duration_minutes * 60
                    'educational_value': float(video[5] / 100) if video[5] else 0.5,  # xp_reward normalized
                    'quality_score': 1.0 - (float(video[6]) / 10) if video[6] else 0.5,  # difficulty_level normalized
                    'url': video[7] or f"https://www.youtube.com/watch?v={video[0]}",  # url
                    'thumbnail_url': '',  # not available in schema
                    'channel': video[8] or '',  # channel_name
                    'difficulty_level': self._map_difficulty_to_text(video[6])  # difficulty_level
                }
    ...
    def _map_difficulty_to_text(self, difficulty_level: int) -> str:
        """Map numeric difficulty level to text"""
