# setup-dev-simple.ps1 - Script simple para configurar el entorno de desarrollo
...
foreach ($port in $ports) {
    ...
    if ($connection) {
        $usedPorts += $port
        Write-Host "Puerto $port esta en uso por PID: $($connection.OwningProcess)" -ForegroundColor Red
    } else {
...
if ($usedPorts.Count -gt 0) {
    Write-Host "Los siguientes puertos estan en uso:" -ForegroundColor Red
    $usedPorts | ForEach-Object { Write-Host "   - Puerto $_" -ForegroundColor Red }
    Write-Host "Cierra las aplicaciones que usen estos puertos o cambia la configuracion" -ForegroundColor Yellow
...
if (!(Test-Path $envPath)) {
    ...
    Write-Host "Archivo .env.local ya existe" -ForegroundColor Yellow
}
...
foreach ($dir in $directories) {
    if (!(Test-Path $dir)) {
        ...
        Write-Host "Directorio ya existe: $dir" -ForegroundColor Yellow
    }
}

# 4. Migrar imágenes
Write-Host "Migrando imagenes..." -ForegroundColor Yellow
if (Test-Path "scripts/migrate-images.js") {
    try {
        node scripts/migrate-images.js
        Write-Host "Migracion de imagenes completada" -ForegroundColor Green
    } catch {
        Write-Host "Error en migracion de imagenes: $_" -ForegroundColor Red
    }
} else {
    Write-Host "Script de migracion no encontrado" -ForegroundColor Yellow
}

# 5. Verificar componente separator
Write-Host "Verificando componente separator..." -ForegroundColor Yellow
$separatorPath = "apps/frontend/app/components/ui/separator.tsx"
if (!(Test-Path $separatorPath)) {
    Write-Host "Componente separator.tsx faltante. Creando..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "Docker no esta disponible. Instalalo desde https://docker.com" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "Python no esta disponible. Instalalo desde https://python.org" -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "Node.js no esta disponible. Instalalo desde https://nodejs.org" -ForegroundColor Red
    exit 1
}
...
Write-Host "¡Todo listo para el desarrollo!" -ForegroundColor Green
