-- Enhanced YouTube Catalog Tables (Corrected version with proper UUID foreign keys)
-- Created for video recommendation system
-- Compatible with existing schema that uses UUID for primary keys

-- Enhanced YouTube Catalog table with semantic search capabilities
CREATE TABLE IF NOT EXISTS youtube_catalog (
    ...
    -- Educational content classification (using UUID to match existing schema)
...
-- Student video interactions tracking (using UUID for user references)
CREATE TABLE IF NOT EXISTS student_video_interactions (
    id SERIAL PRIMARY KEY,
    student_id UUID REFERENCES users(id) ON DELETE CASCADE,
    video_id INTEGER REFERENCES youtube_catalog(id) ON DELETE CASCADE,
    question_id UUID REFERENCES questions(id) ON DELETE SET NULL, -- Question that led to this recommendation
...
CREATE OR REPLACE FUNCTION find_similar_videos_fallback(
    question_text TEXT,
    question_subject_id UUID DEFAULT NULL,
    question_topic_id UUID DEFAULT NULL,
...
-- Get some subject and topic IDs for sample data
DO $$
DECLARE
    math_subject_id UUID;
    sample_topic_id UUID;
BEGIN
    -- Try to get a math subject ID
    SELECT id INTO math_subject_id FROM subjects WHERE LOWER(name) LIKE '%matemátic%' OR LOWER(name) LIKE '%math%' LIMIT 1;
    
    -- If no math subject found, get any subject
    IF math_subject_id IS NULL THEN
        SELECT id INTO math_subject_id FROM subjects LIMIT 1;
    END IF;
    
    -- Try to get a topic from that subject
    IF math_subject_id IS NOT NULL THEN
        SELECT id INTO sample_topic_id FROM topics WHERE subject_id = math_subject_id LIMIT 1;
    END IF;
    ...
    INSERT INTO youtube_catalog (
        ...
        subject_id, topic_id, difficulty_level, quality_score, educational_value, engagement_score,
    ...
    RAISE NOTICE 'Sample data inserted with subject_id: %, topic_id: %', math_subject_id, sample_topic_id;
END $$;
...
-- Final verification
SELECT 'Enhanced YouTube Catalog tables created successfully (corrected UUID version)' as status;